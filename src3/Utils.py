...
class Utils:
    ...
    @staticmethod
    def ts_to_flt(tt:str): # 0:00:02.338000
        tt=str(tt)
        ms=tt.split('.')[1][:3]
        ss=tt.split('.')[0].split(':')[-1]
        mm=tt.split('.')[0].split(':')[-2]
        hh=tt.split('.')[0].split(':')[-3]
        return float(hh)*60*60+float(mm)*60+float(ss)+float(ms)/1000
...
#                os.rename(fp,fp+'_old_'+Utils().get_cur_ts())
    ...
    @staticmethod
    def clear_dir(dir_path, save_fps=[]):
        save_filenames=[x.split('\\')[-1] for x in save_fps]
        if os.path.exists(dir_path):
            for filename in os.listdir(dir_path):
                file_path = os.path.join(dir_path, filename)
                filename=file_path.split('\\')[-1]
                
                if filename not in save_filenames:
